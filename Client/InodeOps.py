...
class Table_Inode():
	def __init__(self, type):
		...
		self.ser_numbers = [-1 for _ in range(((config.INODE_SIZE - 63 - config.MAX_FILE_NAME_SIZE) / 2))]
		self.directory = dict()
...
class Array_Inode():
	def __init__(self, type):
		self.inode = [None for _ in range(9)]
		...
		if self.inode[0] == 0:
			...
			self.inode[8] = [-1 for _ in range(max_data_blocks_allocated)]
		else:
...
class InodeOperations():
	...
	def convert_array_to_table(self, array_inode):
		...
		if array_inode[0] == 0: 
			...
			for i in range(len(array_inode[8])): table_inode.ser_numbers[i] = array_inode[8][i]
		else:
	...
	def convert_table_to_array(self, table_inode):
		...
		if table_inode.type == 0:
			for i in range(len(table_inode.blk_numbers)):
				...
				array.inode[8][i] = table_inode.ser_numbers[i]
		else:
