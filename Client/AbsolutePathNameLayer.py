...
class AbsolutePathNameLayer():
	...
	def new_entry_MFS(self, path, type):
		if path == '/':   #SPECIAL CASE OF INITIALIZING FILE SYSTEM
			interface.new_entry_MFS('/', -1, type)
			return 
		...
		if inode_number_of_parent == -1: 
			...
			return -1
		inode=interface.new_entry_MFS(path[1:], inode_number_of_parent, type)
		return inode
	...
	def new_entry(self, path, type):
		...
		if inode_number_of_parent == -1: 
			print("Error AbsolutePathLayer: Wrong Path!")
			return -1
		inode=interface.new_entry(path[1:], inode_number_of_parent, type)
		return inode
